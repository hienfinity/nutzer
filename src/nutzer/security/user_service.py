...
from nutzer.config import keycloak_admin_config
...
class UserService:
    ...
    def __init__(self) -> None:
        ...
        try:
            ...
            roles_nutzer = [role for role in roles if role["name"] == "nutzer"]
            # Rolle 'nutzer' in Keycloak fuer KeycloakAdmin.assign_client_role() und
            # ist ein dict mit Schluesseln "id", "name", "description", ...
            self.rolle_nutzer = roles_nutzer[0]
            logger.debug("rolle_nutzer={}", self.rolle_nutzer)
        except KeycloakConnectionError:
            logger.error("Keine Verbindung zu Keycloak! Ist Keycloak gestartet?")
            self.client_uuid = "N/A"
            self.rolle_nutzer = None
    ...
    def create_user(self, user: User) -> str:
        ...
        self.keycloak_admin.assign_client_role(
            user_id=user_id, client_id=self.client_uuid, roles=[self.rolle_nutzer]
        )
        return user_id
