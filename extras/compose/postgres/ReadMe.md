...
Original-Verzeichnis `init\nutzer\sql` bzw. `init\tls` in das Named Volume
...
das Named Volume `pg_init` kopiert. Danach wird das Verzeichnis `/tablespace/nutzer`
...
DB `nutzer` mit dem DB-User `nutzer` anzulegen. Mit dem 2. Skript wird das
Schema `nutzer` mit dem DB-User `nutzer` als _Owner_ angelegt:

```shell
    docker compose exec db bash
        psql --dbname=postgres --username=postgres --file=/init/nutzer/sql/create-db.sql
        psql --dbname=nutzer --username=nutzer --file=/init/nutzer/sql/create-schema.sql
        exit
    docker compose down
```
...
Mit der Erweiterung _PostgreSQL_ für VS Code kann man die Datenbank `nutzer` und
...
Im Untermenü _Databases_ von der Verbindung sieht man dann z.B. die Datenbank `nutzer`
mit dem gleichnamigen Schema `nutzer` und die Datenbank `postgres`.
Ebenso kann man man unter _Roles_ den DB-User `nutzer` und den Superuser `postgres`
...
- `nutzer` als Name der kuenftigen DB-Struktur fuer das Projekt.
