...
```shell
    ...
    docker run -v kc_tls:/opt/keycloak/tls -v ./tls:/tmp/tls:ro \
      ...
      cp /tmp/tls/* /opt/keycloak/tls/ -v ./tls:/tmp/tls:ro
...
Konfigurationsdatei `src\nutzer\config\resources\app.toml` in der _Table_ `[keycloak]`
eingetragen:
...
Die Defaultwerte sind in `src\nutzer\config\keycloak.py` definiert.

## Client Secret

In der Datei `src\nutzer\config\resources\app.toml` muss man `client-secret` auf
...
Diese Zeichenkette wird auch für `extras\compose\nutzer\app.toml` benötigt.
